...
contract KYCControl is Ownable {
	...
	/*function KYCUnapprove(address _userAddress) internal {
		isKYCApproved[_userAddress] = false;
		IsKYCApprovedLog(_userAddress, false);
	}*/
}

contract VernamToken is Ownable, KYCControl,ERC20 {
	...
	function VernamToken() public {
		...
		_totalSupply = SafeMath.mul(1000000000,POW);      					// Update total supply
		balances[owner] = SafeMath.mul(500000000,POW);
		_circulatingSupply = balances[owner];
	}
	...
	event Mint(address indexed _minter, uint256 value);
	function mintToken(uint256 _mintedAmount) public onlyOwner returns (bool _success) {
        ...
        Transfer(this, owner, _mintedAmount);
		
		Mint(msg.sender, _mintedAmount);
		return true;
    }
